...
class BillClass:
    def __init__(self, root):
        ...
        self.Product_Table = ttk.Treeview(self.ProductFrame3, style='Treeview', columns=(
            "pid", "category", "name", "scheme", "sellingPrice", "qty", "location"))
        ...
        self.CartTable = ttk.Treeview(self.cart_Frame, style='Treeview', columns=("pid", "name", "scheme", "price", "qty"))
        ...
        txt_p_qty = ctk.CTkEntry(self.Add_CartWidgetsFrame, textvariable=self.var_qty,
                                 font=("times new roman", 17), width=120, height=22).place(relx=0.74, rely=0.3)
        ...
        self.footer = ctk.CTkLabel(self.root,
                                   text="Hassan Pharmacy - Pharmacy Management System | Developed By UA\nFor any Technical Issue contact: "
                                        "+923412800010",
    ...
    def show(self):
        conn = sqlite3.connect(database=r'hpd.db')
        cursor = conn.cursor()
        try:
            cursor.execute("SELECT pid, category, name, scheme, sellingPrice, qty, location FROM product WHERE status='Active'")
    ...
    def add_update_cart(self, e):
        try:
            if self.var_pid.get() == '':
                ...
                cart_data = [self.var_pid.get(), self.var_pname.get(), self.scheme.get(), price_cal, self.var_qty.get(), self.var_stock.get(),
        ...
        try:
            ...
            if self.search_cat.get() in self.cat_list:
                for row in self.cart_list:
                    ...
                    price = str(price)
                    self.txt_bill_area.insert(END, "\n " + self.name + "\t\t\t" + self.scheme.get() + "  \t\t" + row[3] + "\t\t" + row[4] + "\tRs." + price)
